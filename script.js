...
analyzeReportBtn.onclick = function () {
    ...
    if ("02" === contentStart) {
        ...
        const enumMap = new Map(SL651IdentifierEnumData.map(item => [item.code, item]));
        ...
        while (length > currentIndex) {
            ...
            const sl651IdentifierEnum = enumMap.get(enCodeEle);
            ...
            switch (dataType) {
                case "Double":
                    ...
                    if ("FF02" === enCodeEle && regexRtuCode(measuringStation)) {
                        break;
...
function contentSub(content, numOne, numTwo, divide, format, type) {
    ...
    // 判断截取的字符串是否包含无效字符 'a-f'，认为是无效数据
    if (val.substring(2).match(/[a-fA-F]/)) {
        return "-65535";
    }
